...
사람은 두 가지 조건이 **동시에** 맞아야 나온다. 카메라가 5 km 안으로 들어오고, 배속이 1시간/초 이하여야 한다.
위성 고도에서는 1.7 m짜리 사람이 한 픽셀도 되지 않기 때문이다.

찾기 번거로우니 현황판의 **사람 보러 가기** 단추를 누르면 한 번에 맞춰준다.
가장 큰 도시로 날아가고, 배속을 1분/초로 낮추고, 밤이면 오전 10시로 시각을 옮긴다.

현황판의 "보이는 사람"이 0명이면 그 아래에 이유가 나온다. 확대가 부족한지, 배속이 빠른지, 모두 잠들었는지.

![조선 한양의 거리](docs/screens/joseon-street.jpg)

밤에는 대부분 집으로 들어가고 7% 정도만 남는다. 도시가 완전히 비지는 않는다.

## 화면 한켠에 띄워두기

관찰하면서 다른 일을 하라고 만든 기능이다. 세 가지 방법이 있고, 위에서부터 손이 덜 간다.

### 1. 앱 안의 "항상 위에" 단추

개입 패널의 **창 → 항상 위에**. 3D 화면과 압축된 시계·인구 표시가 다른 창 위에 뜨는 작은 창으로 옮겨간다.
그 창을 닫으면 원래 창으로 돌아온다. 크롬과 엣지 116 이상에서 동작하고, 지원하지 않는 브라우저에서는 단추가 숨는다.
브라우저가 제공하는 창이라 별도 설치가 필요 없다.

작은 창에서는 마우스로 시점을 조작하고 배속은 아래 단추로 바꾼다. 키보드 단축키는 원래 창에서만 듣는다.

![미니 모드](docs/screens/mini-mode.jpg)

### 2. 미니 모드 + 창 크기 줄이기

창을 620 x 460 보다 작게 줄이면 자동으로 미니 모드가 된다. 큰 패널이 사라지고 아래 한 줄만 남는다.
`M` 키나 **창 → 미니** 로 직접 켜고 끌 수도 있다. 한 번 직접 누르면 자동 전환은 꺼진다.

주소창과 탭까지 없애고 싶으면 앱 창으로 띄운다.

```powershell
.\Run-Windows.cmd -Mini
```

크롬이나 엣지를 `--app` 모드로 560 x 400 크기로 화면 오른쪽에 연다.

### 3. PowerToys 항상 위에

브라우저를 가리지 않고 아무 창이나 고정하고 싶다면 마이크로소프트 PowerToys를 쓴다.
설치 후 창을 고르고 `Win + Ctrl + T` 를 누르면 그 창이 항상 위에 뜬다.
...
| 마우스 휠 | 줌 (위성 고도 ↔ 골목) |
| `M` | 미니 모드 (작은 창용 압축 표시) |
| 사람 보러 가기 | 사람이 보이는 자리와 배속으로 한 번에 이동 |
| 좌클릭 드래그 | 지도 끌기 |
| 우클릭 드래그 | 회전 |
| `Q` `E` | 좌우 회전 |
| `F` | 도읍으로 |
...
미니 모드가 작은 창에서 제대로 접히는지, "사람 보러 가기"가 실제로 사람을 화면에 올리는지,
밤에 거리가 비되 도시가 완전히 죽지는 않는지도 확인한다.
`npm run verify:pip` 은 항상 위에 띄우기 경로를 따로 검증한다. 헤드리스에는 브라우저의 PiP 창이 없으므로
같은 출처 팝업으로 대체하지만, 그 뒤의 코드는 전부 실제 경로다. 스타일시트 복사, 두 번째 캔버스,
두 번째 WebGL 렌더러, 압축 표시, 되돌리기까지 13개 항목이다.

결과는 `docs/verification-report.txt`, 스크린샷은 `docs/screens/`.

마지막 실행: **54개 전부 통과**, 인구는 서기 400년 96%, 1450년 97%, 1900년 98%, 1975년 97%, 2026년 99%.
